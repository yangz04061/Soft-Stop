...
def find_best_match(signal_name, mapping):
    ...
    for key, value in mapping.items():
        try:
            ...
            if key_str == clean_name:
                return str(value)
            if key_str == signal_name:
                return str(value)
        except:
            continue
    
    # Try with _FD variant
    if not clean_name.endswith('_FD'):
        for key, value in mapping.items():
            try:
                key_str = str(key).rstrip('~').strip()
                if key_str == clean_name + '_FD':
                    return str(value)
            except:
                continue
...
for idx in req_df.index:
    ...
    for signal in signals:
        ...
        if hsi_signal:
            ...
            pattern = r'\{([^}]*' + re.escape(signal.lstrip('_')) + r'[^}]*)\}'
...
output_file = base_path / "cubiX-SGMW - System Requirements_IMPROVED.xlsx"
